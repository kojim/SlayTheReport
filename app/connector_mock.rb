...
class RunDataServiceMock
  ...
  def query_by_author(_name)
    $reports
  end

  def get_item(_author, _runid)
    [Run.new($runfiles[0]), $reports[0]]
  end

  def put_item(author, runid, runfile); end

  def update_item(author, runid, title, floor_comment); end
end
...
class TwitterServiceMock
  ...
  def token_authenticate(_token, _secret)
